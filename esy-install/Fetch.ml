...
let collectPackagesOfSolution installspec solution =
  let pkgs, root =
    ...
    and collectDependencies (seen, topo) pkg =
      let dependencies = Solution.dependencies solution installspec pkg in
  ...
  let%bind () =
    ...
    let rec visit' seen pkg =
      let%bind dependencies =
        RunAsync.List.mapAndJoin
          ~f:(visit seen)
          (Solution.dependencies solution installspec pkg)
      in
    ...
    let%bind rootDependencies =
      RunAsync.List.mapAndJoin
        ...
        (Solution.dependencies solution installspec root)
    in
