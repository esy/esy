...
function calculateEnvironment(sandbox, packageInfo, options = {}) {
  ...
  function setUpBuiltinVariables(envConfigState) {
    ...
    envConfigState = addEnvConfigForPackage(envConfigState, sandbox.packageInfo.rootDirectory, "EsySandBox", curRootPackageJsonOnEjectingHost, mapObject(options.useLooseEnvironment ? sandbox.looseEnv : sandbox.env, env => ({
      val: env,
      exclusive: false,
