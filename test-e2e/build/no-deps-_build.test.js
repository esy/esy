...
const fixture = [
  packageJson({
    ...
    esy: {
      ...
      build: [
        'mkdir -p _build',
        ['cp', '#{self.original_root /}test.ml', './_build/test.ml'],
        ['ocamlopt', '-o', './_build#{/ self.name}.exe', './_build/test.ml'],
      ],
      install: [`cp ./_build/#{self.name}.exe $cur__bin/$cur__name${exeExtension}`],
    },
    dependencies: {
      ocaml: '*',
    },
