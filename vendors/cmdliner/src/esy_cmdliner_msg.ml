...
let quote = Esy_cmdliner_base.quote

let pp = Format.fprintf
let pp_text = Esy_cmdliner_base.pp_text
let pp_lines = Esy_cmdliner_base.pp_lines
...
let err_pos_miss a = match Esy_cmdliner_info.arg_docv a with
...
| [a] -> err_pos_miss a
| args ->
    let add_arg acc a = match Esy_cmdliner_info.arg_docv a with
    | "" -> "ARG" :: acc
    | argv -> argv :: acc
    in
    let rev_args = List.sort Esy_cmdliner_info.rev_arg_pos_cli_order args in
    let args = List.fold_left add_arg [] rev_args in
    let args = String.concat ", " args in
    strf "required arguments %s are missing" args

let err_pos_parse a ~err = match Esy_cmdliner_info.arg_docv a with
| "" -> err
| argv ->
...
let err_arg_missing a =
  if Esy_cmdliner_info.arg_is_pos a then err_pos_miss a else
  strf "required option %s is missing" (Esy_cmdliner_info.arg_opt_name_sample a)

(* Other messages *)

let exec_name ei = Esy_cmdliner_info.(term_name @@ eval_main ei)

let pp_version ppf ei = match Esy_cmdliner_info.(term_version @@ eval_main ei) with
| None -> assert false
| Some v -> pp ppf "@[%a@]@." Esy_cmdliner_base.pp_text v

let pp_try_help ppf ei = match Esy_cmdliner_info.eval_kind ei with
| `Simple | `Multiple_main ->
    pp ppf "@[<2>Try `%s --help' for more information.@]" (exec_name ei)
| `Multiple_sub ->
    let exec_cmd = Esy_cmdliner_docgen.plain_invocation ei in
...
let pp_err_usage ppf ei ~err =
  pp ppf "@[<v>%s: @[%a@]@,@[Usage: @[%a@]@]@,%a@]@."
    (exec_name ei) pp_text err (Esy_cmdliner_docgen.pp_plain_synopsis ~errs:ppf) ei
    pp_try_help ei
