...
let test_exn_backtrace () = (* Don't move this test in the file. *)
  ...
  | ex ->
      ...
      let esy_fmt = Esy_fmt.strf "%a" Esy_fmt.exn_backtrace (ex,bt) in
      assert begin match Printexc.backtrace_status () with
      | false -> esy_fmt = "Exception: Failure(\"Test\")\nNo backtrace available."
      | true ->
          esy_fmt = "Exception: Failure(\"Test\")\n\
...
let test_dump_uchar () =
 let str u = Format.asprintf "%a" Esy_fmt.Dump.uchar u in
...
let test_utf_8 () =
  ...
  assert (Esy_fmt.utf_8 ppf = true);
  Esy_fmt.set_utf_8 ppf false;
  assert (Esy_fmt.utf_8 ppf = false);
  Esy_fmt.set_utf_8 ppf true;
  assert (Esy_fmt.utf_8 ppf = true);
  ()

let test_style_renderer () =
  let ppf = Format.formatter_of_buffer (Buffer.create 23) in
  assert (Esy_fmt.style_renderer ppf = `None);
  Esy_fmt.set_style_renderer ppf `Ansi_tty;
  assert (Esy_fmt.style_renderer ppf = `Ansi_tty);
  Esy_fmt.set_style_renderer ppf `None;
  assert (Esy_fmt.style_renderer ppf = `None);
  ()

let test_exn_typechecks () =
  let (_ : bool) = true || Esy_fmt.failwith "%s" "" in
  let (_ : bool) = true || Esy_fmt.invalid_arg "%s" "" in
  ()

let test_kstr_str_like_partial_app () =
  let assertf f = assert (f "X" = f "X") in
  let test_kstrf esy_fmt = Esy_fmt.kstr (fun x -> x) esy_fmt in
  let test_strf_like esy_fmt = Esy_fmt.str_like Esy_fmt.stderr esy_fmt in
...
let test_byte_size () =
  let size s = Esy_fmt.str "%a" Esy_fmt.byte_size s in
  assert (size 0 = "0B");
...
let test_uint64_ns_span () =
  let span s = Esy_fmt.str "%a" Esy_fmt.uint64_ns_span (Int64.of_string s) in
  assert (span "0u0" = "0ns");
