...
let setup ?style_renderer ?utf_8 oc =
  let ppf =
    if oc == Stdlib.stdout then Esy_fmt.stdout else
    if oc == Stdlib.stderr then Esy_fmt.stderr else
  ...
  in
  Esy_fmt.set_style_renderer ppf style_renderer;
  Esy_fmt.set_utf_8 ppf utf_8;
  ppf
