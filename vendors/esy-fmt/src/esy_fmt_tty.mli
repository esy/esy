...
(** [Esy_fmt] TTY setup.
    ...
    [Esy_fmt_tty] provides simple automatic setup on channel formatters for:
    {ul
    {- {!Esy_fmt.set_style_renderer}. [`Ansi_tty] is used if the channel
       {{!Unix.isatty}is a tty} and the environment variable
       [TERM] is defined and its value is not ["dumb"]. [`None] is
       used otherwise.}
    {- {!Esy_fmt.set_utf_8}. [true] is used if one of the following
       environment variables has ["UTF-8"] as a case insensitive
       substring: [LANG], [LC_ALL], [LC_CTYPE].}} *)

(** {1:tty_setup TTY setup} *)

val setup : ?style_renderer:Esy_fmt.style_renderer -> ?utf_8:bool ->
  out_channel -> Format.formatter
(** [setup ?style_renderer ?utf_8 outc] is a formatter for [outc] with
    {!Esy_fmt.set_style_renderer} and {!Esy_fmt.set_utf_8} correctly setup. If
    [style_renderer] or [utf_8] are specified they override the automatic
    setup.

    If [outc] is {!stdout}, {!Esy_fmt.stdout} is returned. If [outc] is
    {!stderr}, {!Esy_fmt.stderr} is returned. *)

val setup_std_outputs : ?style_renderer:Esy_fmt.style_renderer -> ?utf_8:bool ->
  unit -> unit
