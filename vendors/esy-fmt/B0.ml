...
let esy_cmdliner = B0_ocaml.libname "esy_cmdliner"
...
let fmt_cli =
  ...
  let requires = [esy_cmdliner; fmt] in
...
let default =
  let meta =
    ...
    |> add B0_opam.Meta.depopts ["base-unix", ""; "esy_cmdliner", ""]
    ...
    |> add B0_opam.Meta.build
      {|[["ocaml" "pkg/pkg.ml" "build" "--dev-pkg" "%{dev}%"
          ...
          "--with-esy_cmdliner" "%{esy_cmdliner:installed}%"]]|}
  in
