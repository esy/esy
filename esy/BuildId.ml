...
module Repr = struct
  ...
  type commands =
    | EsyCommands of EsyInstall.Package.CommandList.t
    | OpamCommands of opamcommands
    | NoCommands

  let commands_to_yojson = function
    | NoCommands -> `Null
    | EsyCommands commands -> EsyInstall.Package.CommandList.to_yojson commands
  ...
  let commands_of_yojson =
    ...
    | `List _ as json ->
      let%map commands = EsyInstall.Package.CommandList.of_yojson json in
  ...
  and build = {
    ...
    patches : patch list;
    substs : Path.t list;
    exportedEnv : EsyInstall.Package.ExportedEnv.t;
    buildEnv : EsyInstall.Package.Env.t;
  }
